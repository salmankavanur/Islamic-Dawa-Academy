...
import blogPostsStatic from "@/data/blogData"; // Import static dummy data
...
async function fetchBlogPosts() {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/admin/blog?published=true`, {
    ...
    return data.posts; // Return posts array from API response
  } catch (error) {
...
export default async function BlogPage() {
  ...
  // Use database posts if available and not empty; otherwise, fall back to static data
  const blogPosts = blogPostsDb.length > 0 ? blogPostsDb : blogPostsStatic;

  // Filter featured posts from the final dataset
